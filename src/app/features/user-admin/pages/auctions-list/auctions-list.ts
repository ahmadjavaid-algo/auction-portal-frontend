...
export class AuctionsList {
  ...
  onSearch(): void {
    ...
    if (this.paginator) { this.paginator.firstPage(); this.pageIndex = 0; }
  }
    ...
    ref.afterClosed().subscribe((res) => {
      ...
      if (res.action === 'create') {
        this.aucSvc.add(res.payload).subscribe({
          next: (id) => { this.snack.open(`Auction created (ID ${id}).`, 'OK', { duration: 2500 }); this.loadAuctions(); },
          error: (e) => this.snack.open(e?.error?.message || 'Failed to create auction.', 'Dismiss', { duration: 3000 })
        });
      }
    });
  }
    ...
    ref.afterClosed().subscribe((res) => {
      ...
      if (res.action === 'edit') {
        this.aucSvc.update(res.payload).subscribe({
          next: (ok) => {
            if (ok) { this.snack.open('Auction updated.', 'OK', { duration: 2000 }); this.loadAuctions(); }
            else this.snack.open('Failed to update auction.', 'Dismiss', { duration: 3000 });
          },
          error: (e) => this.snack.open(e?.error?.message || 'Failed to update auction.', 'Dismiss', { duration: 3000 })
        });
      }
    });
  }
  ...
  toggleActive(a: Auction): void {
    ...
    const payload = { AuctionId: a.auctionId, Active: newState, ModifiedById: this.auth.currentUser?.userId ?? null };
    this.aucSvc.activate(payload).subscribe({
      next: (ok) => {
        if (ok) { a.active = newState; this.snack.open(`Auction ${newState ? 'activated' : 'deactivated'}.`, 'OK', { duration: 2000 }); this.computeStats(); }
        else this.snack.open('Failed to change status.', 'Dismiss', { duration: 3000 });
      },
      error: () => this.snack.open('Failed to change status.', 'Dismiss', { duration: 3000 })
    });
  }

  viewAuction(auctionId: number): void { this.router.navigate(['/admin/auctions', auctionId]); }
}
