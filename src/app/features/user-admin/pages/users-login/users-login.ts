...
@Component({
  ...
  styleUrl: './users-login.scss'
})
export class UsersLogin implements AfterViewInit, OnDestroy {
  ...
  async login(): Promise<void> {
    ...
    this.auth.login(this.email, this.password).subscribe({
      next: (resp) => {
        this.loading = false;
        if (resp?.success) this.router.navigate(['/admin/dashboard']);
        else this.error = resp?.message || 'Access denied.';
      },
      error: (err) => {
        this.loading = false;
        this.error = err?.error?.message || 'Connection failed.';
      }
    });
  }
