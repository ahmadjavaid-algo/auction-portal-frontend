...
@Component({
  ...
  styleUrl: './bidders-list.scss'
})
export class BiddersList {
  ...
  getCreatedAt(u: Bidder): Date | null {
  ...
  editUser(row: Bidder): void {
    this.usersSvc.getById(row.userId).subscribe({
      next: (full) => {
        const ref = this.dialog.open<BiddersForm, { mode: 'edit'; initialData: Bidder }, BidderFormResult>(BiddersForm, {
          width: '820px',
  ...
  toggleActive(u: Bidder): void {
    ...
    this.usersSvc.activateUser(payload as Bidder).subscribe({
      next: (ok) => {
        if (ok) {
          u.active = newState;
