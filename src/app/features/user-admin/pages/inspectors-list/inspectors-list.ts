...
@Component({
  ...
  styleUrl: './inspectors-list.scss'
})
export class InspectorsList {
  ...
  openCreateUser(): void {
    const ref = this.dialog.open<InspectorsForm, { mode: 'create' }, InspectorFormResult>(InspectorsForm, {
      width: '820px',
      data: { mode: 'create' }
    });
  ...
  editUser(row: Inspector): void {
    this.usersSvc.getById(row.userId).subscribe({
      next: (full) => {
        const ref = this.dialog.open<InspectorsForm, { mode: 'edit'; initialData: Inspector }, InspectorFormResult>(InspectorsForm, {
          width: '820px',
  ...
  toggleActive(u: Inspector): void {
    ...
    this.usersSvc.activateUser(payload as Inspector).subscribe({
      next: (ok) => {
        if (ok) {
          u.active = newState;
