  ...
  private load(): void {
    ...
    .pipe(
      map(({ auctions, invAucs, files, invs, products }) => {
        ...
        this.title = [year, make, model].filter(Boolean).join(' ') || (this.inventory?.displayName ?? 'Listing');
        ...
        this.images = (files || [])
          .filter(f => (f.active ?? true) && f.inventoryId === this.inventory?.inventoryId && f.documentUrl && isImg(f.documentUrl, f.documentName))
          .map(f => f.documentUrl!)
          .slice(0, 32);
        ...
        const categoryName  = this.product?.categoryName ?? snap?.Category ?? snap?.category ?? null;
        ...
        const byScore = (x: InventoryAuction) => {
          ...
          const prod = inv ? (products || []).find(p => p.productId === inv.productId) || null : null;
        ...
        this.related = allLots
          .filter(x => ((x as any).inventoryAuctionId ?? (x as any).inventoryauctionId) !== this.inventoryAuctionId)
          .map(x => {
            const inv = (invs || []).find(i => i.inventoryId === x.inventoryId) || null;
            const prod = inv ? (products || []).find(p => p.productId === inv.productId) || null : null;
            ...
            const title = [yy, mk, md].filter(Boolean).join(' ') || (inv?.displayName ?? 'Listing');
            const img = (imageMap.get(x.inventoryId) || [])[0] || (this.images[0] || '');
            const chassis2 = inv?.chassisNo || snap2?.Chassis || '';
            const sub = chassis2 ? `Chassis ${chassis2} • #${(x as any).inventoryAuctionId ?? ''}` : `#${(x as any).inventoryAuctionId ?? ''}`;
            return {
              link: ['/bidder/auctions', ((x as any).auctionId ?? currentAuctionId), ((x as any).inventoryAuctionId ?? (x as any).inventoryauctionId)],
              title, imageUrl: img, sub,
              buyNow: x.buyNowPrice ?? null, reserve: x.reservePrice ?? null,
              _score: byScore(x)
            } as any;
          })
    ...
    .subscribe({
      ...
      error: () => { this.error = 'Failed to load listing.'; this.loading = false; }
    });
  }
